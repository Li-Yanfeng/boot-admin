...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public void saveUser(UserDTO resource) {
        ...
        if (CollUtil.isNotEmpty(resource.getRoles())) {
            Set<RoleSmallDTO> roles = resource.getRoles();
            roles.forEach(role -> {
        ...
        if (CollUtil.isNotEmpty(resource.getJobs())) {
            Set<JobSmallDTO> jobs = resource.getJobs();
            jobs.forEach(job -> {
    ...
    private void getRelevantData(UserDTO userDTO) {
        ...
        userDTO.setRoles(CollUtil.newHashSet(roleService.listRolesByUserId(userId)));
        userDTO.setJobs(CollUtil.newHashSet(jobService.listJobsByUserId(userId)));
