...
import org.utility.mapper.ColumnConfigMapper;
...
public class ColumnConfigServiceImpl extends ServiceImpl<ColumnConfigMapper, ColumnConfig> implements ColumnConfigService {
    ...
    @Override
    public void download(GenConfig genConfig, List<ColumnConfig> columnConfigs, HttpServletRequest request, HttpServletResponse response) {
        if (genConfig.getConfigId() == null) {
            throw new BadRequestException("请先配置生成器");
        }
        try {
